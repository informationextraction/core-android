...
public class Device {
	...
	public boolean isSimulator() {
		...
		return android.os.Build.MODEL.endsWith(Messages.getString("Device.0")); //$NON-NLS-1$
	}
	...
	public String getImei() {
		...
		if (imei == null || imei.length() == 0) {
			...
			if (imei == null || imei.length() == 0) {
				imei = Messages.getString("Device.4"); //$NON-NLS-1$
			}
		}
	...
	public String getImsi() {
		...
		if (imsi == null) {
			imsi = Messages.getString("Device.1"); //$NON-NLS-1$
		}
	...
	public static CellInfo getCellInfo() {
		...
		if (bcell == null) {
			if (Cfg.DEBUG) {
				Check.log(TAG + Messages.getString("Device.6") + Messages.getString("Device.7")); //$NON-NLS-1$ //$NON-NLS-2$
			}
			return info;
		}
		...
		if (bcell instanceof GsmCellLocation) {
			if (Cfg.DEBUG) {
				Check.asserts(Device.isGprs(), Messages.getString("Device.8")); //$NON-NLS-1$
			}
			...
			if (Cfg.DEBUG) {
				Check.log(TAG + Messages.getString("Device.9") + info.toString()); //$NON-NLS-1$
			}
		...
		if (bcell instanceof CdmaCellLocation) {
			if (Cfg.DEBUG) {
				Check.asserts(Device.isCdma(), Messages.getString("Device.10")); //$NON-NLS-1$
			}
			...
			if (Cfg.DEBUG) {
				Check.log(TAG + Messages.getString("Device.11") + info.toString()); //$NON-NLS-1$
			}
